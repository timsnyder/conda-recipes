...
run_test() {
	...
	cmp "$OUTPUT" "$TESTDIR/res${test_num}.txt" >> $LOGFILE 2>&1
	if [ $? -eq 0 ]; then
...
if [ -n "$FONTDIR" ]; then
    ...
    FIGLET_FONTDIR=$($CMD -I2)
fi
export FIGLET_FONTDIR
printf "Current font dir: "; $CMD -I2
printf "Default font: "; $CMD -I3
$CMD -f small "Package Test results" | tee -a $LOGFILE

run_test 001 "showfigfonts output" "showfigfonts"
run_test 002 "text rendering in all fonts" \
  "for i in ${FIGLET_FONTDIR}/*.flf; do $cmd -f \$i; done"
run_test 003 "long text rendering" "cat ${TESTDIR}/longtext.txt|$CMD"
...
run_test 012 "tlf2 font rendering" "$cmd -f ${TESTDIR}/emboss"
...
run_test 017 "tlf2 kerning flush-right mode" "$cmd -kr -f ${TESTDIR}/emboss"
run_test 018 "tlf2 overlap centered mode" "$cmd -oc -f ${TESTDIR}/emboss"
run_test 019 "tlf2 full-width flush-left right-to-left mode" \
  "$cmd -WRl -f ${TESTDIR}/emboss"
run_test 020 "specify font directory" \
  "X=`mktemp -d`;cp $FIGLET_FONTDIR/script.flf \$X/foo.flf;$cmd -d\$X -ffoo;rm -Rf \$X"
...
run_test 024 "list of control files" "ls $FIGLET_FONTDIR/*flc | xargs -n1 basename"
